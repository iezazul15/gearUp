...
const createPaymentIntent = async (userId: string, rentalOrderId: string) => {
  ...
  const data: SSLCommerzPayment = {
    ...
    currency: "BDT",
    tran_id: `${trnx_id}`,
    success_url: `${config.frontend_url}/api/payments/callback?tran_id=${trnx_id}&order_id=${rentalOrder.id}&status=success`,
    fail_url: `${config.frontend_url}/api/payments/callback?tran_id=${trnx_id}&order_id=${rentalOrder.id}&status=fail`,
    cancel_url: `${config.frontend_url}/api/payments/callback?tran_id=${trnx_id}&order_id=${rentalOrder.id}&status=cancel`,
