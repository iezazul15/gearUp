...
const createPaymentIntent = async (userId: string, rentalOrderId: string) => {
  ...
  const data: SSLCommerzPayment = {
    ...
    currency: "BDT",
    tran_id: `${trnx_id}`,
    success_url: `http://localhost:${config.port}/api/payments/callback?tran_id=${trnx_id}&order_id=${rentalOrder.id}&status=success`,
    fail_url: `http://localhost:${config.port}/api/payments/callback?tran_id=${trnx_id}&order_id=${rentalOrder.id}&status=fail`,
    cancel_url: `http://localhost:${config.port}/api/payments/callback?tran_id=${trnx_id}&order_id=${rentalOrder.id}&status=cancel`,
